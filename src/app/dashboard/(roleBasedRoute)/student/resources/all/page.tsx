...
export default function TeacherMyResourcesPage() {
  ...
  useEffect(()=>{ fetch("/api/resource/categories",{credentials:"include"}).then(r=>r.json()).then(d=>{ if(d.success) setCategories(d.data??[]); }); },[]);
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-5 lg:p-7 pt-6 max-w-6xl mx-auto">
      ...
      {!loading && <p className="text-[12.5px] text-muted-foreground -mt-2">{meta.total} resource{meta.total!==1?"s":""} found</p>}
              ...
              <div className="px-4 pt-4 pb-3 flex-1 flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-border mt-auto">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {/* <a href={viewUrl(r.fileUrl)} target="_blank" rel="noreferrer" className="p-1.5 rounded-lg text-muted-foreground/40 hover:text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-950/20 transition-colors" title="Open in new tab">
                      <RiExternalLinkLine className="text-[15px]"/>
                    </a> */}
      ...
      {meta.totalPages>1 && (
        <div className="flex items-center justify-center gap-2 pt-2">
          <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page<=1} className="px-4 py-2 rounded-xl border border-border text-[12.5px] font-semibold text-muted-foreground hover:bg-muted/40 disabled:opacity-40 transition-colors">Previous</button>
          <span className="text-[12.5px] text-muted-foreground px-2">Page {page} of {meta.totalPages}</span>
          <button onClick={()=>setPage(p=>Math.min(meta.totalPages,p+1))} disabled={page>=meta.totalPages} className="px-4 py-2 rounded-xl border border-border text-[12.5px] font-semibold text-muted-foreground hover:bg-muted/40 disabled:opacity-40 transition-colors">Next</button>
        </div>
      )}
    </div>
  );
